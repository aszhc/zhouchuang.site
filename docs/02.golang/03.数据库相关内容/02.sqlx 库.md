---
title: sqlx 库
date: 2024-12-14 15:21:14
permalink: /pages/29646f/
categories:
  - golang
tags:
  - 数据库
...
`sqlx` 是一个 Go 语言的扩展库，它建立在 Go 的标准库 `database/sql` 之上，提供了更多功能和便利性。

## sqlx 介绍

`sqlx`可以认为是Go语言内置`database/sql`的超集，它在优秀的内置`database/sql`基础上提供了一组扩展。这些扩展中除了大家常用来查询的`Get(dest interface{}, ...) error`和`Select(dest interface{}, ...) error`外还有很多其他强大的功能。
...
```go
type User struct {
	Id   int    `db:"id"`   // 结构体字段必须是公共的
	Age  int    `db:"age"`
	Name string `db:"name"`
}
```

1. **结构体字段必须是公共的：** `sqlx` 通过反射来访问结构体的字段，只有公共字段（首字母大写）才能被反射访问。
2. **使用结构体标签：** 结构体标签可以帮助 `sqlx` 更好地映射数据。
